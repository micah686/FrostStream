...
<section class={card}>
  ...
  <form class="grid gap-5 lg:grid-cols-2" onsubmit={next}>
    ...
    <div class="flex justify-between gap-3 lg:col-span-2"><a href="/admin/import" class="btn btn-sm btn-neutral text-xs">Cancel</a><button class="btn btn-sm btn-primary text-xs" type="submit" disabled={busy}>{#if busy}<span class="loading loading-spinner loading-xs mr-2"></span>{/if}Scan files and continue</button></div>
  </form>
</section>
