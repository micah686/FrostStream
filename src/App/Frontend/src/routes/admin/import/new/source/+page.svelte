...
<section class={card}>
  ...
  <form class="grid gap-5 lg:grid-cols-2" onsubmit={next}>
    ...
    <div><Label for="worker" class="mb-2 text-slate-300">Worker tag</Label><Input id="worker" list="worker-tags" bind:value={workerTag} placeholder="Optional — search tags" class={field} /><datalist id="worker-tags">{#each workerTags as tag}<option value={tag}></option>{/each}</datalist><Helper class="mt-1 text-xs! text-slate-500!">Tags are reported by available workers. Leave blank when any worker can handle the import.</Helper></div>
    <div><Label class="mb-2 text-slate-300">Available workers</Label><div class="min-h-10 rounded-lg border border-slate-800 bg-slate-950/60 px-3 py-2 text-xs text-slate-400">{#if visibleWorkers.length}{visibleWorkers.map((worker) => worker.name).join(', ')}{:else}No matching workers online{/if}</div></div>
    <div class="lg:col-span-2"><Label for="subpath" class="mb-2 text-slate-300">Source folder</Label><div class="flex gap-2"><Input id="subpath" bind:value={subPath} placeholder="Select a folder on the chosen worker" class="{field} min-w-0 flex-1" /><Button type="button" color="dark" class="border-slate-700! bg-slate-900!" onclick={() => (pickerOpen = true)} disabled={!visibleWorkers.length}><FolderOpenOutline class="mr-2 h-4 w-4" />Browse</Button></div></div>
