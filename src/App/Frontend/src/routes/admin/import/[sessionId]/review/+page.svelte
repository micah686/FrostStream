...
<section class={card}>
  ...
  {#if session?.deleteSourceFiles}
    <div class="mb-5 flex items-start gap-3 rounded-box border-[length:var(--border)] border-warning/30 bg-warning/10 p-4">
  ...
  {/if}
  {#if loading}<div class="flex items-center gap-2 p-8 text-sm text-base-content/60"><span class="loading loading-spinner loading-xs"></span>Loading review…</div>{:else}<div class="space-y-2">{#each items as item (item.itemId)}<div class="card border-[length:var(--border)] border-base-300 bg-base-100 p-4"><div class="flex items-center gap-3"><div class="min-w-0 flex-1"><p class="truncate text-sm font-medium text-base-content/90">{item.title || item.fileName}</p><p class="truncate text-xs text-base-content/50" title={item.relativePath}>{item.relativePath}</p></div><span class={`badge badge-sm shrink-0 ${pill(item)}`}>{label(item)}</span></div>{#if item.metadataSource === 'manualMapping'}<details class="mt-3 border-t border-base-300 pt-3"><summary class="cursor-pointer text-xs font-semibold text-base-content/60">Show mapped metadata</summary><pre class="mt-3 max-h-72 overflow-auto whitespace-pre-wrap rounded-field bg-black/25 p-3 text-xs text-white/70">{pretty(item.metadataJson)}</pre></details>{/if}{#if item.metadataFetchState === 'failed'}<p class="mt-2 text-xs text-error">yt-dlp failed; {label(item).toLowerCase()} metadata will be used instead.</p>{/if}</div>{:else}<p class="rounded-box border-[length:var(--border)] border-dashed border-base-300 p-8 text-center text-sm text-base-content/50">No files selected.</p>{/each}</div>{/if}
