<script lang="ts">
  ...
  function pill(item: ImportSessionItem) { if (item.metadataSource === 'ytDlp' || item.hasInfoJson) return 'bg-success/10 text-success'; if (item.metadataSource === 'manualMapping') return 'bg-secondary/10 text-secondary'; if (item.metadataSource === 'nfo' || item.metadataSource === 'infoJson') return 'bg-primary/10 text-primary'; return 'bg-base-200 text-base-content/60'; }
...
<section class={card}>
  ...
  {/if}
  {#if loading}<div class="flex items-center gap-2 p-8 text-sm text-base-content/60"><span class="loading loading-spinner loading-xs"></span>Loading review…</div>{:else}<div class="space-y-2">{#each items as item (item.itemId)}<div class="rounded-xl border border-base-300 bg-base-200/25 px-4 py-3"><div class="flex items-center gap-3"><div class="min-w-0 flex-1"><p class="truncate text-sm font-medium text-base-content/90">{item.title || item.fileName}</p><p class="truncate text-xs text-base-content/50" title={item.relativePath}>{item.relativePath}</p></div><span class={`shrink-0 rounded-full px-2.5 py-1 text-xs font-semibold ${pill(item)}`}>{label(item)}</span></div>{#if item.metadataSource === 'manualMapping'}<details class="mt-3 border-t border-base-300 pt-3"><summary class="cursor-pointer text-xs font-semibold text-base-content/60">Show mapped metadata</summary><pre class="mt-3 max-h-72 overflow-auto whitespace-pre-wrap rounded-lg bg-black/25 p-3 text-xs text-base-content/60">{pretty(item.metadataJson)}</pre></details>{/if}{#if item.metadataFetchState === 'failed'}<p class="mt-2 text-xs text-warning">yt-dlp failed; {label(item).toLowerCase()} metadata will be used instead.</p>{/if}</div>{:else}<p class="rounded-xl border border-dashed border-base-300 p-8 text-center text-sm text-base-content/50">No files selected.</p>{/each}</div>{/if}
  <div class="mt-6 flex justify-between"><a class="rounded-lg px-4 py-2.5 text-sm font-semibold text-base-content/60 hover:text-base-content" href={`/admin/import/${sessionId}/mapping`}>Back</a><a class="rounded-lg bg-primary px-6 py-2.5 text-sm font-semibold text-primary-content hover:bg-primary" href={`/admin/import/${sessionId}/run`}>Next: import</a></div>
</section>
