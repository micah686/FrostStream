...
<section class={card}>
  <div class="flex flex-wrap items-start gap-3"><div><h1 class="text-xl font-bold text-base-content">File selection</h1><p class="mt-2 text-sm text-base-content/60">Choose only the media files you want this session to import.</p></div>{#if session}<div class="ml-auto rounded-full bg-base-200 px-3 py-1.5 text-xs font-semibold text-base-content/80">{selectedCount} selected · {session.totalItems} found</div>{/if}</div>
  ...
  {:else}
    <div class="mb-4 flex gap-2"><input class="input w-full" bind:value={search} placeholder="Filter by filename or path" /><button class="btn btn-sm btn-neutral" onclick={() => load()}>Search</button></div>
    ...
    <div class="mt-5 flex items-start gap-3 rounded-xl border border-base-300 bg-base-200/30 p-4">
      <input type="checkbox" class="toggle" checked={session?.deleteSourceFiles ?? false} onchange={(e) => setDeleteSourceFiles(e.currentTarget.checked)} />
      <div class="min-w-0">
