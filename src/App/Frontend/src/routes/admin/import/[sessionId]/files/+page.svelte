...
<section class={card}>
  ...
  {#if session?.status === 'scanning'}
    <div class="flex items-center justify-center gap-3 rounded-box border-[length:var(--border)] border-base-300 bg-base-200/30 p-12 text-sm text-base-content/60"><span class="loading loading-spinner loading-sm"></span>Scanning the selected folder…</div>
  {:else}
    ...
    <div class="card mt-5 flex items-start gap-3 border-[length:var(--border)] border-base-300 bg-base-100 p-4">
