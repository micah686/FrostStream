<script lang="ts">
  ...
  function stateClass(item: ImportSessionItem) {
    if (item.metadataFetchState === 'queued') return 'bg-primary/10 text-primary';
    if (localInfoJsonFound(item)) return 'bg-success/10 text-success';
...
<section class={card}>
  ...
  <div class="mt-6 flex justify-between"><a class="rounded-lg px-4 py-2.5 text-sm font-semibold text-base-content/60 hover:text-base-content" href={`/admin/import/${sessionId}/files`}>Back</a><a class="rounded-lg bg-primary px-6 py-2.5 text-sm font-semibold text-primary-content hover:bg-primary" href={`/admin/import/${sessionId}/mapping`}>Next: manual mapping</a></div>
</section>
