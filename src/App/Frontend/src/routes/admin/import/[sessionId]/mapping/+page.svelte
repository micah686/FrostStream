...
<section class={card}>
  <div class="flex flex-wrap items-start gap-3"><div><h1 class="text-xl font-bold text-white">Manual mapping</h1><p class="mt-2 max-w-3xl text-sm text-slate-400">Download a realistic example for reference, or generate a sparse mapping file for the selected files, edit it, then import it here.</p></div><div class="ml-auto flex flex-wrap gap-2"><a href={mappingExampleUrl(sessionId)} download><Button color="dark" class="border-slate-700! bg-slate-900! text-xs!">Download example JSON</Button></a><a href={mappingTemplateUrl(sessionId)} download><Button color="dark" class="border-slate-700! bg-slate-900! text-xs!" disabled={!needsMapping.length}>Generate mapping file</Button></a><input class="hidden" bind:this={fileInput} type="file" accept=".json,.csv,application/json,text/csv" onchange={upload} /><Button color="blue" class="border-0! text-xs!" disabled={busy} onclick={() => fileInput?.click()}>{#if busy}<Spinner size="4" class="mr-2" />{/if}Import mapping</Button></div></div>
