...
<section class={card}>
  ...
  <div class="grid gap-5 xl:grid-cols-2">
    ...
    <div class="overflow-hidden rounded-xl border border-base-300"><div class="bg-base-200/50 px-4 py-3"><h2 class="font-semibold text-base-content/90">Metadata resolved <span class="text-base-content/50">({resolved.length})</span></h2></div><div class="max-h-[440px] divide-y divide-base-300 overflow-y-auto">{#each resolved as item (item.itemId)}<div class="bg-base-200/20 px-4 py-3 opacity-70"><div class="flex items-center gap-2"><p class="min-w-0 flex-1 truncate text-sm text-base-content/80">{item.fileName}</p><span class={`badge badge-sm shrink-0 ${item.metadataFetchState === 'failed' ? 'badge-error' : item.metadataSource === 'ytDlp' || item.hasInfoJson ? 'badge-success' : 'badge-neutral'}`}>{pill(item)}</span></div><p class="truncate text-xs text-base-content/40" title={item.relativePath}>{item.relativePath}</p></div>{:else}<p class="p-8 text-center text-sm text-base-content/50">No mappings applied yet.</p>{/each}</div></div>
  </div>
  <div class="mt-6 flex justify-between"><a class="btn btn-sm btn-neutral text-xs" href={`/admin/import/${sessionId}/metadata`}>Back</a><a class="btn btn-sm btn-primary text-xs" href={`/admin/import/${sessionId}/review`}>Next: review</a></div>
</section>
