...
/// mid-read; they are then trimmed to <see cref="MaxCompletedRetained"/> newest.
/// </summary>
    ...
    : BackgroundService
{
    ...
    /// <summary>Current snapshot: running runs first (oldest first), then recently completed (newest first).</summary>
    public IReadOnlyList<BackgroundRunView> List()
    {
        Prune();
        return _runs.Values
            .OrderByDescending(x => x.Status == "running")
            .ThenBy(x => x.Status == "running" ? x.StartedAt : Instant.MaxValue)
    ...
    /// <summary>Drops finished runs once they age out, keeping only the newest few for context.</summary>
    private void Prune()
    {
        var now = clock.GetCurrentInstant();
        var finished = _runs.Values
            .Where(x => x.Status != "running" && x.CompletedAt is not null)
