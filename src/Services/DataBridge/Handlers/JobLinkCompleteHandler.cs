...
public class JobLinkCompleteHandler : OneWayMessageHandlerBase<JobLinkCompleteRequest>
{
    ...
    protected override string Subject => Subjects.JobLinkComplete;

    protected override async Task HandleRequestAsync(
        FrostStreamDbContext db,
        JobLinkCompleteRequest request,
        CancellationToken cancellationToken)
    {
        var version = await db.VideoVersions.AsNoTracking()
            .FirstOrDefaultAsync(v => v.Id == request.ExistingVersionId, cancellationToken);
