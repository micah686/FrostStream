...
public class StreamingNetworkHandler : IStorageHandler
{
    ...
    public StreamingNetworkHandler(ILogger<StreamingNetworkHandler> logger)
    {
        _logger = logger;
    }
        ...
        CancellationToken ct)
    {
        if (string.IsNullOrEmpty(config.ConnectionString))
            throw new InvalidOperationException($"Job {job.JobId}: StreamingNetwork requires a connection string");
